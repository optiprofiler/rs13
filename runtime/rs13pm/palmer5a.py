import bam

def objective(x):
    f = 0.0
    v = [0.0] * (4)
    v[0] = -x[1]
    v[0] += x[0]
    v[0] += x[2]
    v[1] = -x[3]
    v[0] += v[1]
    v[0] += x[4]
    v[1] = -x[5]
    v[0] += v[1]
    v[1] = x[6] / x[7]
    v[0] += v[1]
    v[1] = 83.57418 - v[0]
    v[0] = v[1] * v[1]
    v[1] = 1.0000000000000009 * x[1]
    v[1] += x[0]
    v[2] = 1.0000000000000027 * x[2]
    v[1] += v[2]
    v[2] = 1.0000000000000062 * x[3]
    v[1] += v[2]
    v[2] = 1.000000000000011 * x[4]
    v[1] += v[2]
    v[2] = 1.0000000000000173 * x[5]
    v[1] += v[2]
    v[2] = 2.4674000736160004 + x[7]
    v[3] = x[6] / v[2]
    v[1] += v[3]
    v[3] = 81.007654 - v[1]
    v[1] = v[3] * v[3]
    v[0] += v[1]
    v[1] = 0.5802466620760969 * x[1]
    v[1] += x[0]
    v[3] = -0.3266276222990955 * x[2]
    v[1] += v[3]
    v[3] = -0.9592958372379013 * x[3]
    v[1] += v[3]
    v[3] = -0.7866287927024783 * x[4]
    v[1] += v[3]
    v[3] = 0.046418374720775146 * x[5]
    v[1] += v[3]
    v[3] = 1.949550365169 + x[7]
    v[2] = x[6] / v[3]
    v[1] += v[2]
    v[2] = 18.983286 - v[1]
    v[1] = v[2] * v[2]
    v[0] += v[1]
    v[1] = 0.38888959624442854 * x[1]
    v[1] += x[0]
    v[2] = -0.6975297638656907 * x[2]
    v[1] += v[2]
    v[2] = -0.9314137327208286 * x[3]
    v[1] += v[2]
    v[2] = -0.026904457042947638 * x[4]
    v[1] += v[2]
    v[2] = 0.9104880058476136 * x[5]
    v[1] += v[2]
    v[2] = 1.7134731460089998 + x[7]
    v[3] = x[6] / v[2]
    v[1] += v[3]
    v[3] = 8.051067 - v[1]
    v[1] = v[3] * v[3]
    v[0] += v[1]
    v[1] = 0.20987610307763616 * x[1]
    v[1] += x[0]
    v[3] = -0.9119040427138908 * x[2]
    v[1] += v[3]
    v[3] = -0.5926498368087033 * x[3]
    v[1] += v[3]
    v[3] = 0.6631379662358756 * x[4]
    v[1] += v[3]
    v[3] = 0.8710034611215323 * x[5]
    v[1] += v[3]
    v[3] = 1.4926241929 + x[7]
    v[2] = x[6] / v[3]
    v[1] += v[2]
    v[2] = 2.044762 - v[1]
    v[1] = v[2] * v[2]
    v[0] += v[1]
    v[1] = 0.027400834407416585 * x[1]
    v[1] += x[0]
    v[2] = -0.9984983885475546 * x[2]
    v[1] += v[2]
    v[2] = -0.08212021240874423 * x[3]
    v[1] += v[2]
    v[2] = 0.9939980638641267 * x[4]
    v[1] += v[2]
    v[2] = 0.13659296510721142 * x[5]
    v[1] += v[2]
    v[2] = 1.2675044472249999 + x[7]
    v[3] = x[6] / v[2]
    v[1] += v[3]
    v[3] = -v[1]
    v[1] = v[3] * v[3]
    v[0] += v[1]
    v[1] = -0.11110997934203104 * x[1]
    v[1] += x[0]
    v[3] = -0.9753091449812269 * x[2]
    v[1] += v[3]
    v[3] = 0.3278431372439471 * x[3]
    v[1] += v[3]
    v[3] = 0.9024558565680235 * x[4]
    v[1] += v[3]
    v[3] = -0.5283868404046833 * x[5]
    v[1] += v[3]
    v[3] = 1.0966236512040002 + x[7]
    v[2] = x[6] / v[3]
    v[1] += v[2]
    v[2] = 1.170451 - v[1]
    v[1] = v[2] * v[2]
    v[0] += v[1]
    v[1] = -0.38271526343196616 * x[1]
    v[1] += x[0]
    v[2] = -0.7070580542724016 * x[2]
    v[1] += v[2]
    v[2] = 0.9239190824370774 * x[3]
    v[1] += v[2]
    v[2] = -0.0001378157770514088 * x[4]
    v[1] += v[2]
    v[2] = -0.9238135940342387 * x[5]
    v[1] += v[2]
    v[2] = 0.7615442022250001 + x[7]
    v[3] = x[6] / v[2]
    v[1] += v[3]
    v[3] = 10.479881 - v[1]
    v[1] = v[3] * v[3]
    v[0] += v[1]
    v[1] = -0.6049377685964584 * x[1]
    v[1] += x[0]
    v[3] = -0.26810059225107563 * x[2]
    v[1] += v[3]
    v[3] = 0.9293061166679677 * x[3]
    v[1] += v[3]
    v[3] = -0.8562441448692452 * x[4]
    v[1] += v[3]
    v[3] = 0.10664272807399988 * x[5]
    v[1] += v[3]
    v[3] = 0.487388289424 + x[7]
    v[2] = x[6] / v[3]
    v[1] += v[2]
    v[2] = 25.785001 - v[1]
    v[1] = v[2] * v[2]
    v[0] += v[1]
    v[1] = -0.7777774948355076 * x[1]
    v[1] += x[0]
    v[2] = 0.20987566294519644 * x[2]
    v[1] += v[2]
    v[2] = 0.45130436013059516 * x[3]
    v[1] += v[2]
    v[2] = -0.9119044122066285 * x[4]
    v[1] += v[2]
    v[2] = 0.9672130983804402 * x[5]
    v[1] += v[2]
    v[2] = 0.27415591280100005 + x[7]
    v[3] = x[6] / v[2]
    v[1] += v[3]
    v[3] = 44.126844 - v[1]
    v[1] = v[3] * v[3]
    v[0] += v[1]
    v[1] = -0.9012344421491146 * x[1]
    v[1] += x[0]
    v[3] = 0.6244470394316517 * x[2]
    v[1] += v[3]
    v[3] = -0.22431191631858705 * x[3]
    v[1] += v[3]
    v[3] = -0.2201317898900903 * x[4]
    v[1] += v[3]
    v[3] = 0.6210926180403503 * x[5]
    v[1] += v[3]
    v[3] = 0.121847072356 + x[7]
    v[2] = x[6] / v[3]
    v[1] += v[2]
    v[2] = 62.822177 - v[1]
    v[1] = v[2] * v[2]
    v[0] += v[1]
    v[1] = -0.9753086105372786 * x[1]
    v[1] += x[0]
    v[2] = 0.902453771576314 * x[2]
    v[1] += v[2]
    v[2] = -0.7850332575231643 * x[3]
    v[1] += v[2]
    v[2] = 0.6288456196646282 * x[4]
    v[1] += v[2]
    v[2] = -0.44160383759196054 * x[5]
    v[1] += v[2]
    v[2] = 0.030461768089 + x[7]
    v[3] = x[6] / v[2]
    v[1] += v[3]
    v[3] = 77.719674 - v[1]
    v[1] = v[3] * v[3]
    v[0] += v[1]
    f = v[0]
    return f


xmin = [-1000000, -100000, -10000, -10000, -10000, -10000, 0, 1e-05]
xmax = [1000000, 100000, 10000, 10000, 10000, 10000, 10000000, 10000]
x0 = [0, 0, 0, 0, 0, 0, 5000, 5000]

res = bam.minimize(objective, x0=x0,
                   bounds=list(zip(xmin, xmax)),
                   options={"tracefname": "palmer5a.trc"})

print("Status = %d" % res.status)
print("Best f = %.15e" % res.fun)
print("Best x = " + "".join("%.15e " % xi for xi in res.x))
